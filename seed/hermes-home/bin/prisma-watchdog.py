...
def should_skip(entry: dict, signature: str, cooldown_minutes: int) -> bool:
...
def select_candidate(stalled_minutes: int, cooldown_minutes: int, state_db: dict) -> tuple[pathlib.Path | None, dict | None]:
    ...
    for review_dir in list_review_dirs():
        ...
        if should_skip(entry, signature, cooldown_minutes):
            continue
...
def resume_review(review_dir: pathlib.Path, runtime_state: dict, state_db: dict, models: list[str], agent_timeout: int) -> None:
    review_key = str(review_dir)
    entry = state_db.setdefault("reviews", {}).setdefault(review_key, {})
    signature = candidate_signature(runtime_state)
    ...
    if attempts + 1 < len(models):
        ...
        send_telegram_message(
            "Watchdog PRISMA\n"
            f"La reanudación de `{review_dir.name}` no se completó y ya se agotó la cadena de modelos prevista.\n\n"
            f"{summary}\n\n"
            f"Salida: {(result.stderr or result.stdout or '').strip()[:800]}"
        )


def pass_once(stalled_minutes: int, cooldown_minutes: int, models: list[str], agent_timeout: int, dry_run: bool = False) -> int:
    state_db = load_state()
    review_dir, runtime_state = select_candidate(stalled_minutes, cooldown_minutes, state_db)
    ...
    if dry_run:
        ...
        return 0
    resume_review(review_dir, runtime_state, state_db, models, agent_timeout)
    return 0
